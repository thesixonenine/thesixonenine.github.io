...
const WishHistoryFilePath = "F:\\game\\Genshin Impact\\Genshin Impact Game\\YuanShen_Data\\webCaches\\"
const JSONFilePath = "../assets/data/genshin-impact.json"
const GachaUrlPrefix = "https://public-operation-hk4e.mihoyo.com/gacha_info/api/getGachaLog"
...
func FetchWishes(urlParam UrlParam, localHistoryMap map[string][]types.MiHoYoWish) {
    ...
    // 循环抽卡类型
    for k, v := range constant.HK4EGachaType {
        fmt.Printf("开始获取[%s]\n", v)
        localIdList := MapToId(localHistoryMap[k])
        // 如果是新手跃迁且已经抽了50抽则直接跳过
        if k == "100" && len(localIdList) == 50 {
            continue
        }
        page := 1
        size := 5
        ...
        for {
            ...
            for _, wish := range wishList {
                ...
                fmt.Println(wish.Fmt(constant.HK4EGachaType))
            }
            if isContains {
                break
            }
